...
function SkeletonRow({ columns, rowIndex }: { columns: DataTableColumn[], rowIndex: number }) {
    ...
    const rowBg = isOdd ? "#fff1f2" : "#ffffff"
    return (
...
function TablePaginationSkeleton() {
    return (
        <div className="flex items-center justify-between px-4 py-3 bg-[#7a0f1f] border-b border-white/20">
            <div className="h-5 w-32 md:w-48 bg-white/20 rounded animate-pulse" />

            <div className="flex items-center gap-1 opacity-60">
                <div className="h-8 w-8 bg-white/20 rounded-md animate-pulse" />
                <div className="h-8 w-6 bg-white/20 rounded-md animate-pulse" />
                <div className="h-8 w-6 bg-white/20 rounded-md animate-pulse" />
                <div className="h-8 w-8 bg-white/20 rounded-md animate-pulse" />
            </div>
        </div>
...
function TablePagination({
    ...
    const btnBase = "inline-flex items-center justify-center h-8 min-w-[32px] px-2 rounded-md text-sm font-medium transition-colors"
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-[#7a0f1f] text-white border-b border-white/20">
            ...
            {last_page > 1 && (
                <div className="flex items-center gap-1">
                    ...
                    {pages.map((p, i) =>
                        p === "..." ? (
                            <span key={`ellipsis-${i}`} className="px-1 text-xs text-white/50">…</span>
                        ) : (
                            <button
                                key={p}
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {loading ? (
                            ...
                            displayRows.map((row, rowIndex) => {
                                ...
                                const isOdd = rowIndex % 2 === 1
                                const rowBg = selected ? "#fecdd3" : isOdd ? "#fff1f2" : "#ffffff"
                                return (
                                    <tr
                                        key={row.id ?? rowIndex}
                                        className={`group transition-colors ${selected ? "border-l-2 border-l-[#7a0f1f]" : ""} ${clickable ? "cursor-pointer" : ""}`}
                                        style={{ background: rowBg }}
                                        onMouseEnter={(e) => { if (!selected) (e.currentTarget as HTMLTableRowElement).style.background = isOdd ? "#fecdd3" : "#fff1f2" }}
